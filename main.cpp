...
int main(){
    ...
    if(!handover_version&&!is_attacker){
        write1= fopen("results1_std.xls", "w");
        write2= fopen("results2_std.xls", "w");
    }
    else if(handover_version&&!is_attacker){
        write1= fopen("results1_patch.xls", "w");
        write2= fopen("results2_patch.xls", "w");
    }
    else if(handover_version&&is_attacker){
        write1= fopen("results1_patch_att.xls", "w");
        write2= fopen("results2_patch_att.xls", "w");
        write3= fopen("results3_patch_att.xls", "w");
    }
    else{
        puts("ERROR: wrong scenario definition");
        return 1;
    }
    ...
    int j=0;
    while(overall_time1.size() < n_rounds || overall_time2.size() < n_rounds){
